...
function calculate_grid_centre(squares_per_row, square_size) { // returns 3D vector corresponding to centre of surface of grid
  
    let half_grid = squares_per_row / 2 * square_size.x;
    return createVector(half_grid, 0, -half_grid); 
  }
  
  function calculate_grid_width(blocks_per_row, block_size) { // in pixels
    let grid_width_pixels = blocks_per_row * block_size.x;
