...
function compute_elements() { 
  ...
  // // Draw elements
  // draw_grid_surface(y_animate, grid.x_squares, grid.y_squares, grid.square_size, grid.stroke_colour, grid.start_colour);
  // draw_frame();
  // draw_water(water);
  // draw_tree(c02);
...
function compute_grid_surface(farmland_m2){
  ...
  if(y_squares < x_squares){ // ensure min grid visualisation in the case of small values 
    y_squares = x_squares;
  }
...
function draw_grid_surface(y_limit, x_squares, y_squares, square_size, grid_width_pixels, stroke_colour, start_colour) {
  ...
  noFill();
  strokeWeight(0.4);
  for (let x=x_squares-1; x>0; x--) { // x-axis   
    for (let y=1; y<=y_squares; y++) { // y-axis
      
      if(y_limit > y) { // set y limit for animation 
          beginShape();
          if(y>x_squares){
             stroke_colour = lerpColor(start_colour, background_colour_RGB, y/y_squares);
          }else{
...
function draw_water(water) {
  ...
  let inner_circle_size = map(water, 0, WATER_MAX, outer_circle_size*0.95, outer_circle_size*0.7); 
  ...
  torus(outer_circle_size/2, (outer_circle_size-inner_circle_size)/2, 36, 4);
...
function draw_frame() {
  push();
  translate(grid_centre_pixels.x/2, grid_centre_pixels.y, grid_centre_pixels.z); // move to centre
